...
pal_nds_save_slot_offset(
   ...
   uint32_t *offset)
{
   if (!pal_nds_save_available || offset == NULL ||
...
pal_nds_save_read(
   ...
   uint32_t size)
{
   return NdsRetail_BackupRead(offset, destination, size);
}
...
pal_nds_save_write(
   ...
   uint32_t size)
{
   return NdsRetail_BackupWrite(offset, source, size);
}
...
PalTargetSave_Init(
   void)
{
   /* Retail software knows its backup protocol and capacity; it does not
    * probe the chip at runtime. Loaders provision and redirect this fixed
    * 1MiB type-3 FLASH image through the ARM7 SDK backup surface. */
   pal_nds_save_available = NdsRetail_BackupInit();
   if (pal_nds_save_available)
   {
      pal_nds_save_log("save: retail FLASH 1MiB");
   }
   else
   {
      pal_nds_save_log("save: Slot-1 backup unavailable");
   }
...
PalTargetSave_Type(
   void)
{
   return PAL_NDS_SAVE_TYPE_FLASH_1M;
}
   ...
   uint16_t *saved_times)
{
   ...
   if (!pal_nds_save_read(base, count, sizeof(count)))
   {
      return false;
   }
   ...
   size_t *out_size)
{
   ...
   while (offset < footer.payload_size)
   {
      ...
      if (!pal_nds_save_read(base + offset, output + offset, amount))
      {
   ...
   size_t size)
{
   ...
   {
      if (!NdsRetail_BackupErase(base + offset))
      {
         pal_nds_save_log("save w%d: erase fail @%lx",
   ...
   for (offset = 0u; offset < size; offset += PAL_NDS_SAVE_IO_BYTES)
   {
      ...
      if (!pal_nds_save_write(base + offset, input + offset, amount))
      {
         pal_nds_save_log("save w%d: write fail @%lx",
            slot, (unsigned long)offset);
         return false;
      }
      NdsTarget_AudioPump();
   }
   if (!pal_nds_save_write(
         base + PAL_NDS_SAVE_FOOTER_OFFSET, &footer, sizeof(footer)))
   {
      pal_nds_save_log("save w%d: footer write fail", slot);
      return false;
   }
   pal_nds_save_log("save w%d: ok %lu bytes",
