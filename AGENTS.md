...
- The CoreS3 SE bring-up now runs a bounded scene-enter/auto/trigger-script subset directly against mutable scene and event-object records. Supported operations include stop/jump/wait/random branch, nested trigger calls, scene/map transition, palette day/night switches, party placement/composition/followers/trail sync, basic NPC/party movement, viewport movement, player-sprite/event-state/event-pointer updates, and visual updates used by the scene scaffold. It also renders script dialog text overlays from the preconverted UTF-16 text/font caches into the indexed framebuffer; the next touch clears the dialog and is consumed before movement/scene hot zones. The audited `/mnt/hgfs/deb13/PAL` scene-enter/auto/trigger roots now have no first-op unsupported scripts in the bounded scan; battle/inventory/audio/fade/RNG commands still advance as placeholders while those systems are out of scope. Center tapping triggers the facing event through the same subset. Unsupported script operations deliberately leave the script pointer unchanged until the full interpreter is ported.
