...
- The CoreS3 SE bring-up now runs a small bounded scene-enter/auto/trigger-script subset directly against mutable scene and event-object records. Supported operations are basic stop/jump/wait, scene transition, palette day/night switches, party placement/composition/trail sync, and visual/event-pointer updates used by the scene scaffold; common audio/dialog/fade/shake commands advance as no-ops while audio/dialog UI is out of scope. Center tapping triggers the facing event through the same subset. Unsupported script operations deliberately leave the script pointer unchanged until the full interpreter is ported.
