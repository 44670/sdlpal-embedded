...
- `embedded/pal_pack.c` is the first native runtime slice following the contract: no heap, no decompressor, `const uint8_t` pack reads, TF-pack TOC copy into fixed PSRAM, fixed `uint8_t` copy destination, and objdump/nm/size verification through `embedded/Makefile`.
...
- `embedded/pal_realdata_sdl_smoke.c` is the native SDL real-data smoke test. It maps generated NOR/TF packs read-only, copies the TF pack TOC into `pal_psram_tf_toc`, copies TF chunks into named SRAM/PSRAM arrays, reads real save files into static PSRAM storage, and is run with `make -C embedded realdata-check`, which rebuilds the packs first.
- `make -C embedded contract-check` runs the embedded source/binary/pack budget gate with heap/decoder scans, generated-pack checks, 16MB NOR pack budget, `.rodata`/`.bss` section budgets, and named `pal_sram_`, `pal_psram_`, `pal_scene_`, `pal_battle_`, `pal_sfx_`, `pal_audio_`, `pal_global_`, `pal_save_`, `pal_video_`, `pal_ui_`, `pal_music_`, `pal_menu_`, `pal_ending_`, `pal_palette_`, and `pal_dialog_` symbol totals.
- `make -C unix EMBEDDED_CONTRACT=1` builds a reduced native SDL2 full-engine profile named `unix/sdlpal-embedded-contract`. It excludes MP3/OGG/OPUS/AVI/Timidity/TSF/GLSL/native-MIDI launcher paths plus desktop sound/RIX/resampler/adplug/font/text/codepage paths, but it is still not contract-clean because full-engine heap and YJ paths remain.
- `embedded/pal_scene_cache.c` is the static scene-loading slice. It copies decoded MAP/GOP chunks into named PSRAM arrays, deduplicates event-object MGO sprites as `const uint8_t *` references into the NOR pack, and can pin one scene's unique MGO sprites into `pal_psram_sprite_pin` for TF-backed sprite placement.
- `embedded/pal_battle_cache.c` is the static battle-loading slice. It copies decoded FBP backgrounds into PSRAM, keeps F/ABC/FIRE sprites plus `DATA.MKF #10` battle effects as `const uint8_t *` views into the NOR pack, can stage one FIRE effect in `pal_psram_effect`, and deduplicates enemy sprite references.
