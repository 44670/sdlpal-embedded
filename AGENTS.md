...
- `embedded/pal_video_static.c` is the static indexed-video slice. It uses `pal_sram_framebuffer`, `pal_psram_screen_bak`, and `pal_sram_display_dma` for clear/full-screen save/rectangular save/restore/scanline RGB565 conversion without `gpScreenReal`, `VIDEO_DuplicateSurface`, or texture-sized project buffers.
