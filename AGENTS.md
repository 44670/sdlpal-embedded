...
- Under the same contract defines, `main.c` splash loading uses `pal_sram_splash_fbp` for FBP staging, keeps the mutable title sprite in `pal_psram_splash_title`, and maps the crane sprite as a read-only `const uint8_t` NOR view. It only accepts already-native splash FBP/MGO chunks.
