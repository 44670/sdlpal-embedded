...
- Under the same contract defines, `ui.c` keeps `DATA.MKF #9` UI sprite and one box save/restore buffer in named PSRAM storage, so `PAL_InitUI()` and `PAL_CreateBoxInternal()` do not allocate project heap.
