...
- Under the same contract defines, `battle.c` maps F/ABC battle sprites as `const uint8_t` views into the native NOR pack. Battle backgrounds and `DATA.MKF #10` effect storage use fixed PSRAM buffers (`pal_psram_battle_background_static`, `pal_psram_battle_effect_static`), with F/ABC/FBP chunks required to already be native.
