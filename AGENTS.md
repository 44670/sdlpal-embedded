...
- `make -C esp32s3 check` runs the CoreS3 SE artifact gate. In addition to source/ELF/pack/static-buffer checks, it scans the generated NOR `SSS` script data and fails if any `/mnt/hgfs/deb13/PAL` scene-enter/auto/trigger root reaches an opcode that lacks an explicit `execute_script_mutation()` case within the bounded branch-aware scan.
...
- The CoreS3 SE bring-up now runs a bounded scene-enter/auto/trigger-script subset directly against mutable scene, event-object, party, player-role, cash, and inventory records. Supported operations include stop/jump/wait/random branch, nested trigger calls, scene/map/teleport-script transition, palette day/night/palette-number/fade switches, music start/stop/CD-fallback/battlefield state, screen-wave/screen-shake rendering, party placement/composition/followers/trail sync, basic NPC/party/chase movement, viewport movement, player-sprite/event-state/event-pointer updates, visual updates, script dialog overlays from preconverted UTF-16 text/font caches, static buy/sell overlays backed by NOR store/object data plus the fixed inventory save slice, FBP still-picture previews from the TF pack, player HP/MP/stat edits, cash changes, equipment removal, learned-magic edits, and item add/remove/count/equipped/full-HP branches used by the scene scaffold. Battle-start scripts now load the requested enemy team/battlefield through the static battle cache and render a short indexed-framebuffer preview using TF FBP background data plus NOR F/ABC sprite views; this is not the final turn-based battle loop yet. RNG scripts now stream host-predecoded TF RNG frame records through the fixed 65KB PSRAM frame buffers and blit them into the indexed framebuffer without runtime decompression. The next touch clears a dialog and is consumed before movement/scene hot zones. The audited `/mnt/hgfs/deb13/PAL` scene-enter/auto/trigger roots now have no first-op unsupported scripts in the bounded scan; touch-trigger event ranges now run through the same subset, and center tapping handles search-trigger events by trigger mode. Audio commands still advance as placeholders while that system is out of scope. Unsupported script operations deliberately leave the script pointer unchanged until the full interpreter is ported.
...
- The CoreS3 SE bring-up reads startup saves from FatFS into `pal_psram_save_state`; when no save exists, it builds a default save-shaped runtime state in the same fixed buffer from read-only NOR scene/event/player-role templates and targets slot `0:/1.rpg` for later writes. Runtime sync updates scene, viewport, party count/direction/layer, follower count, palette offset, music/battle fields, screen wave, party frame records, and trail records before writes. This is only a save I/O proof for the scene scaffold, not the final menu-driven save system.
