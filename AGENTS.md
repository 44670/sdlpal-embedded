...
- `make -C unix EMBEDDED_CONTRACT=1` builds a reduced native SDL2 full-engine profile named `unix/sdlpal-embedded-contract`. It excludes MP3/OGG/OPUS/AVI/Timidity/TSF/GLSL/native-MIDI launcher paths plus desktop sound/RIX/resampler/adplug paths, but it is still not contract-clean because full-engine heap and YJ paths remain.
