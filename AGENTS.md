...
- The CoreS3 SE bring-up now runs a bounded scene-enter/auto/trigger-script subset directly against mutable scene and event-object records. Supported operations include stop/jump/wait/random branch, nested trigger calls, scene transition, palette day/night switches, party placement/composition/trail sync, basic NPC/party movement, viewport movement, player-sprite/event-state/event-pointer updates, and visual updates used by the scene scaffold; common battle/audio/dialog/fade/shake commands advance as placeholders while those systems are out of scope. Center tapping triggers the facing event through the same subset. Unsupported script operations deliberately leave the script pointer unchanged until the full interpreter is ported.
