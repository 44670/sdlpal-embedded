...
- Under the same contract defines, `battle.c` battle-background and battle-effect storage use static `uint8_t` PSRAM buffers (`pal_psram_battle_background_static`, `pal_psram_battle_effect_static`), with FBP backgrounds required to already be native.
