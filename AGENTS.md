...
- `make -C esp32s3 check` runs the CoreS3 SE artifact gate. In addition to source/ELF/pack/static-buffer checks, it scans the generated NOR `SSS` script data and fails if any `/mnt/hgfs/deb13/PAL` scene-enter/auto/trigger root reaches an opcode that lacks an explicit `execute_script_mutation()` case within the bounded scan.
