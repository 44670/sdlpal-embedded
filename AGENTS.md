...
- The CoreS3 SE bring-up reads startup saves from FatFS into `pal_psram_save_state` and writes the same fixed buffer back to the loaded short-name slot after scene changes and completed enter/trigger script progress. Runtime sync updates scene, viewport, party count/direction/layer, follower count, palette offset, music/battle fields, screen wave, party frame records, and trail records before writes. This is only a save I/O proof for the scene scaffold, not the final menu-driven save system.
