...
If `/sdcard/1.rpg` is present, the firmware reads only its fixed 44-byte common header into static storage and uses the saved scene and viewport as the startup view. The full save body is not loaded on the target path yet, so mutable event state still comes from the default global cache.

The current firmware draws a 320x200 indexed framebuffer through `embedded/pal_video_static.c`, centered on the 320x240 LCD with 20-pixel black bars. Touch input is polled through FT6336; dragging inside the PAL viewport pans the loaded scene while the party leader faces the pan direction, and tapping the top or bottom black bar loads the previous or next scene from the TF pack. This proves the CoreS3 SE LCD/touch/TF/NOR scene path without adding audio or runtime decompression.
