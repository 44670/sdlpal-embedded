...
If `/sdcard/1.rpg` is present, the firmware reads it into the existing 256KB static save buffer and uses its saved scene, viewport, player-role data, scene table, and event-object table as the startup view. If the save is missing or invalid, it falls back to the default mutable global cache.
