...
The current firmware draws a 320x200 indexed framebuffer through `embedded/pal_video_static.c`, centered on the 320x240 LCD with 20-pixel black bars. Touch input is polled through FT6336; holding inside a viewport quadrant moves the party leader by the original 16x8 isometric walking step, with map-tile and blocking event-object collision checks, tapping near the party triggers the facing event through a bounded script subset, and tapping the top or bottom black bar loads the previous or next scene from the TF pack. The bounded scene-enter/auto/trigger-script subset runs on mutable scene, event-object, party, player-role, cash, and inventory records for stop/jump/wait/random branch, nested trigger calls, scene/map transition, palette day/night, party placement/composition/followers/trail sync, basic NPC/party movement, viewport movement, visual/event-pointer/state updates, script dialog overlays, player HP/MP/stat edits, cash changes, and item add/remove/count branches. Script dialog text renders as an indexed-framebuffer overlay using the preconverted UTF-16 text/font caches, and the next touch clears the dialog before movement or scene hot zones run. The audited PAL scene-enter/auto/trigger roots have no first-op unsupported scripts in the bounded scan; battle/audio/fade/RNG commands still advance as placeholders while those UI/audio systems are out of scope. This proves the CoreS3 SE LCD/touch/TF/NOR scene path without adding audio or runtime decompression.
